    ...
    <section class="ppdb-hero-section">
        <div class="ppdb-hero-container">
            ...
            <div class="ppdb-hero-center">
                <div class="ppdb-telah-dibuka-badge">{{ $settings->hero_badge_text }}</div>
                <h1 class="ppdb-hero-main-title">{{ $settings->hero_main_title }}</h1>
                <h2 class="ppdb-hero-sub-title">{{ $settings->hero_sub_title }}</h2>
                <div class="ppdb-hero-academic-year">{{ $settings->hero_academic_year }}</div>

                <div class="ppdb-registration-date-box">
                    <span class="ppdb-date-label">{{ $settings->hero_date_label }}</span>
                    <span class="ppdb-date-value">{{ $settings->hero_date_value }}</span>
                </div>
            </div>
            ...
            <div class="ppdb-hero-right">
                ...
                <div class="ppdb-mini-logo">
                    <div class="ppdb-mini-logo-text">{{ $settings->hero_sub_title }}</div>
                    <div class="ppdb-mini-logo-sub">{{ $settings->hero_logo_sub }}</div>
                </div>
                ...
                <div class="ppdb-social-badges">
                    <div class="ppdb-badge-row">
                        @if ($settings->instagram_url)
                            <a href="{{ $settings->instagram_url }}" target="_blank" class="ppdb-contact-pill instagram">
                        ...
                        @if ($settings->whatsapp_number)
                            <a href="https://wa.me/62{{ ltrim(str_replace(['-', ' '], '', $settings->whatsapp_number), '0') }}" target="_blank" class="ppdb-contact-pill whatsapp">
        ...
        <!-- Major Levels Bar (masih statis dulu, nanti ganti dari tabel jurusans) -->
    ...
    <section class="ppdb-welcome-content">
        <div class="ppdb-welcome-container">
            ...
            <p class="ppdb-welcome-text address-section">
                <strong>Alamat:</strong> {{ $settings->address }}<br>
                <strong>Telp/WA:</strong> {{ $settings->phone }}<br>
                <strong>Website:</strong> <a href="{{ $settings->website_url }}" target="_blank" class="ppdb-link-highlight">{{ $settings->website_url }}</a>,
                <strong>Email:</strong> <a href="mailto:{{ $settings->email }}" class="ppdb-link-highlight">{{ $settings->email }}</a>
            </p>
        </div>
    </section>
@endsection